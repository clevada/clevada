<div class="row gx-5">
    ...
    <div class="col-md-6">
        ...
        <div class="form-group">
            <div class="form-check form-switch">
                <input type='hidden' value='' name='posts_listing_show_category'>
                <input class="form-check-input" type="checkbox" id="posts_listing_show_category" name="posts_listing_show_category" @if ($templateConfig->posts_listing_show_category ?? null) checked @endif>
                <label class="form-check-label" for="posts_listing_show_category">{{ __('Show category') }}</label>
            </div>
        </div>

        
        <div class="fw-bold fs-5 mb-2 mt-3">{{ __('Posts category') }}</div>

        <div class="form-group">
            <div class="form-check form-switch">
                <input type='hidden' value='' name='posts_categ_show_categ_hero'>
                <input class="form-check-input" type="checkbox" id="posts_categ_show_categ_hero" name="posts_categ_show_categ_hero" @if ($templateConfig->posts_categ_show_categ_hero ?? null) checked @endif>
                <label class="form-check-label" for="posts_categ_show_categ_hero">{{ __('Show hero in posts categories pages') }}</label>
                <div class="form-text text-muted small">{{ __('Show hero with category image, category title and description. Note: it works only for categories with image uploaded.') }}</div>
            </div>
        </div>
...
@if (file_exists(resource_path('views/templates/' . $slug . '/tab-posts.blade.php')))
    <hr>
    @include('templates.' . $slug . '.tab-posts')
@endif
