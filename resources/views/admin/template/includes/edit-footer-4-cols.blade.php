            ...
            <div class="builder-col sortable" id="sortable_4">
                <div class="mb-4 text-center">
                    <a class="btn btn-danger" href="#" data-bs-toggle="modal" data-bs-target="#addBlock4"><i class="bi bi-plus-circle"></i>
                        {{ __('Add content block') }}</a>
                    @include('admin.template.modals.add-footer-block', ['col' => 4])
                </div>

                @foreach (footer_blocks($footer, $col = 4, $show_hidden = true) as $block)
                    <div class="builder-block movable" id="item-{{ $block->id }}">
                        <div class="float-end ms-2">
                            @if ($block->hide == 1)
                                <div class="badge bg-danger fs-6 me-2">{{ __('Hidden') }}</div>
            ...
            $("#sortable_4").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: "{{ route('admin.template.footer.sortable', ['footer' => $footer, 'col' => 4]) }}",
                    });
                }
            });
