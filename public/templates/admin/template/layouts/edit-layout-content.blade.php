        ...
        <div class="builder-col sortable" id="sortable">
            ...
            @foreach (template_blocks($module, $template->id, $show_hidden = 1) as $block)
                <div class="builder-block movable" id="item-{{ $block->id }}">
                    ...
                    @if (!($block->type == 'map' || $block->type == 'blockquote' || $block->type == 'spacer' || $block->type == 'include'))@if (!$block->block_content_default_lang)<div class="text-danger">{{ __('Content not added.') }}</div>@endif @endif
