        ...
        <div class="col-md-4 col-12">
            <div class="builder-col sortable" id="sortable_1">
                <div class="mb-4 text-center">
                    ...
                    @include('admin.includes.modal-add-footer-block', ['col' => 1])    
                </div>

                @foreach (footer_blocks($footer, $col = 1, $template->id) as $block)
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <form method="POST" action="{{ route('admin.template.footer.content.delete', ['template_id' => $template->id, 'footer' => $footer, 'block_id' => $block->id]) }}">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
            ...
            <div class="builder-col sortable" id="sortable_2">
                <div class="mb-4 text-center">
                    <a class="btn btn-danger" href="#" data-bs-toggle="modal" data-bs-target="#addBlock2"><i class="bi bi-plus-circle"></i>
                        {{ __('Add content block') }}</a>
                    @include('admin.includes.modal-add-footer-block', ['col' => 2])    
                </div>

                @foreach (footer_blocks($footer, $col = 2, $template->id) as $block)
                    <div class="builder-block movable" id="item-{{ $block->id }}">
                        <div class="float-end ms-2">
                            <a href="{{ route('admin.template.footer.block', ['id' => $block->id]) }}" class="btn btn-primary btn-sm">{{ __('Manage content') }}</a>
                            <a href="#" data-bs-toggle="modal" data-bs-target=".confirm-{{ $block->id }}" class="btn btn-outline-danger btn-sm ms-2"><i class="bi bi-trash"></i></a>
            ...
            <div class="builder-col sortable" id="sortable_3">
                <div class="mb-4 text-center">
                    <a class="btn btn-danger" href="#" data-bs-toggle="modal" data-bs-target="#addBlock3"><i class="bi bi-plus-circle"></i>
                        {{ __('Add content block') }}</a>
                    @include('admin.includes.modal-add-footer-block', ['col' => 3])    
                </div>

                @foreach (footer_blocks($footer, $col = 3, $template->id) as $block)
                    <div class="builder-block movable" id="item-{{ $block->id }}">
                        <div class="float-end ms-2">
                            <a href="{{ route('admin.template.footer.block', ['id' => $block->id]) }}" class="btn btn-primary btn-sm">{{ __('Manage content') }}</a>
                            <a href="#" data-bs-toggle="modal" data-bs-target=".confirm-{{ $block->id }}" class="btn btn-outline-danger btn-sm ms-2"><i class="bi bi-trash"></i></a>
            ...
            $("#sortable_1").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: "{{ route('admin.template.footer.sortable', ['template_id' => $template->id, 'footer' => $footer, 'col' => 1]) }}",
                    });
                }
            });
            ...
            $("#sortable_2").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: "{{ route('admin.template.footer.sortable', ['template_id' => $template->id, 'footer' => $footer, 'col' => 2]) }}",
                    });
                }
            });
            ...
            $("#sortable_3").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: "{{ route('admin.template.footer.sortable', ['template_id' => $template->id, 'footer' => $footer, 'col' => 3]) }}",
                    });
                }
            });
