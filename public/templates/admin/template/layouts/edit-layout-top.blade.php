<div class="row">
    ...
    <div class="col-12">
        <div class="builder-col">
            ...
            <h5>{{ __('Manage top section') }} ({{ __('optional') }})</h5>
            <div class="text-muted small mb-1">{{ __('This section is below the navigation menu and above main content') }}</div>
            ...
            <form action="{{ route('admin.template.global_sections.assign', ['module' => $module ]) }}" method="POST">
                ...
                <select class="form-select form-select-lg" name="section_id">
                    ...
                    @foreach ($global_sections as $global_section)
                        <option @if (($top_section_id ?? null) == $global_section->id) selected @endif value="{{ $global_section->id }}">{{ $global_section->label }}</option>
                    @endforeach
                </select>
                </div>
                ...
                <button class="btn btn-danger mt-3" type="submit">{{ __('Update top content') }}</button>
            </form>
