<div class="modal fade custom-modal" tabindex="-1" role="dialog" aria-labelledby="updateLabel{{ $tag->id }}" aria-hidden="true" id="update-tag-{{ $tag->id }}">
		...
		<div class="modal-content">
			...
			<form action="{{ route('admin.accounts.tags.show', ['id' => $tag->id]) }}" method="post">
				...
				<div class="modal-header">
					<h5 class="modal-title" id="updateLabel{{ $tag->id }}">{{ __('Update account tag') }}</h5>
					...
					<div class="row">
						...
						<div class="col-lg-6">
							<div class="form-group">
								...
								<input class="form-control form-control-color" id="color_{{ $tag->id }}" name="color" value="{{ $tag->color ?? '#000' }}">
                                <script>
