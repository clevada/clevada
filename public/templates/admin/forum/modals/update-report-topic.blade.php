...
<div class="modal fade custom-modal" tabindex="-1" role="dialog" aria-labelledby="update-report-{{ $report->id }}" aria-hidden="true" id="update-report-{{ $report->id }}">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.forum.reports.topic.update', ['id' => $report->id]) }}" method="post">
                @csrf
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-checkbox">
                                    <input id="checkbox_close_topic" type="checkbox" name="close_topic" class="custom-control-input">
                                    <label for="checkbox_close_topic" class="custom-control-label"> {{ __('Close topic') }}</label>
                                </div>
                            </div>

                            <div class="form-group">
                                <div class="custom-control custom-checkbox">
                                    <input id="checkbox_delete_topic" type="checkbox" name="delete_topic" class="custom-control-input" aria-describedby="deleteTopic">
                                    <label for="checkbox_delete_topic" class="custom-control-label text-danger"> {{ __('Delete topic') }}</label>
                                    <small id="deleteTopic" class="form-text text-muted">{{ __('All topic posts will be deleted') }}</small>
                                </div>
                            </div>
