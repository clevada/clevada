<div class="card-header">
	<h3><i class="fas fa-exclamation-triangle"></i> {{ __('Topics reported') }} ({{ $reports_pending ?? 0 }} {{ __('pending') }})</h3>
</div>
<!-- end card-header -->
				...
				<tr>
					...
					<td>
						<div class="mb-2 font-weight-bold">
							{{ __('This topic have') }} {{ forum_topic_reports($report->topic_id)->total() ?? 0 }} {{ __('reports') }}
						</div>
						@foreach(forum_topic_reports($report->topic_id) as $report)
					...
					<td>
						...
						@include('admin.forum.modals.update-report-topic')

						<form method="POST" action="{{ route('admin.forum.reports.topics.delete', ['id' => $report->id]) }}">
							{{ csrf_field() }}
