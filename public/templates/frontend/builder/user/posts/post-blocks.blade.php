                ...
                @foreach (content_blocks('posts', $post->id, $show_hidden = 1) as $block)
                    <div class="builder-block movable" id="item-{{ $block->id }}">
                        <div class="float-end ms-2">
                            ...
                            <a href="{{ route('user.posts.block', ['id' => $block->id]) }}" class="btn btn-primary">{{ __('Manage content') }}</a>                                                    
    ...
    @include('frontend.builder.user.posts.includes.add-content-block', ['module' => 'posts', 'content_id' => $post->id])
            ...
            $("#sortable").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        data: data,
                        type: 'POST',
                        url: "{{ route('user.posts.sortable', ['id' => $post->id]) }}",
                    });
                }
            });
